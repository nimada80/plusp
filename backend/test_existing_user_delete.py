#!/usr/bin/env python
"""
اسکریپت تست برای حذف یک کاربر موجود
"""
...
try:
    ...
    # پیدا کردن یک کاربر موجود
    logger.info("دریافت لیست کاربران موجود")
    users = _make_request('GET', '/rest/v1/users?limit=1')
    
    if not users or len(users) == 0:
        logger.error("هیچ کاربری یافت نشد. نیاز به ایجاد کاربر تست است.")
        ...
        # انتخاب اولین کاربر از لیست
        test_user = users[0]
        test_user_id = test_user.get('uid')
        logger.info(f"کاربر موجود با شناسه {test_user_id} انتخاب شد")
    ...
    if auth_check is None or (isinstance(auth_check, list) and len(auth_check) == 0) or 'error_code' in auth_check:
